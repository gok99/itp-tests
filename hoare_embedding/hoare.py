...
def assign_intro(x: str, a: Callable[[State], int], Q: Condition) -> HoareTriple:
    ...
    Note that the condition Q is substituted with a in x in the PRECONDITION
    """
    return (subst(Q, a, x), Stmt.ASSIGN(x, a), Q)
# Example:
# assign_intro("x", lambda s: s["x"] + 1, lambda s: s["x"] == 1)
# == (lambda s: s["x"] + 1 == 1, Stmt.ASSIGN("x", lambda s: s["x"] + 1), lambda s: s["x"] == 1)
...
#### ORIGINAL PROOF 1
    ...
    # First assign n := n-1
    # {m + n = a + b} n := n-1 {m + (n + 1) = a + b}
    hoare_n = assign_intro("n", lambda s: s["n"] - 1,
        lambda s: s["m"] + (s["n"] + 1) == a + b)
    
    # Then assign m := m+1
    # {(m + 1) + n = a + b} m := m+1 {m + n = a + b}
    hoare_m = assign_intro("m", lambda s: s["m"] + 1,
        lambda s: s["m"] + s["n"] == a + b)
    
    # Combine the assignments
    hoare_body = seq_intro(hoare_n, hoare_m)
    
    # Make the body satisfy the while rule precondition
    # Minor innacuracy
    hoare_body_with_cond = consequence(
        lambda s: s["m"] + s["n"] == a + b and s["n"] != 0,
        hoare_body,
        lambda s: s["m"] + s["n"] == a + b
    )
    
    # Step 2: Apply the while rule
    hoare_while = while_intro(I, B, hoare_body_with_cond)
    
    # Step 3: Connect the precondition to the loop invariant
    # and the postcondition to the final state
    return consequence(
        P,  # {n = a ∧ m = b}
        hoare_while,  # {I} while n ≠ 0 do (n := n-1; m := m+1) {I ∧ ¬B}
        Q   # {n = 0 ∧ m = a + b}
    )
